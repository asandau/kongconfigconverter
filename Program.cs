...
namespace kongconfigconverter
{
    class Program
    {
        ...
        {
            var input = new StreamReader("users.yaml");

            var deserializer = new DeserializerBuilder()
                .WithNamingConvention(new CamelCaseNamingConvention())
                .Build();

            var entity = deserializer.Deserialize<KongfigFormat>(input);

            var newFormat = new KongInitFormat();
            var services = new List<Service>();
            var routes = new List<Route>();
            var plugins = new List<Plugin>();

            foreach (var api in entity.apis) {
            ...
            newFormat.plugins = plugins;
            newFormat.services = services;
            newFormat.routes = routes;

            var serializer = new SerializerBuilder().Build();
            var yaml = serializer.Serialize(newFormat);

            using (StreamWriter outputFile = new StreamWriter( "out.yaml"))
            {
        ...
        public static List<Plugin> mergePlugins(List<Plugin> plugins, List<Plugin> newPlugins) {
        ...
        private static string addTarget(string target, string targetName)
        {
            return $"{target.Split("]")[0]}, {targetName}]";
        }

        public static Plugin existsAlready(List<Plugin> plugins, Plugin plugin) {
        ...
        public static Service addToServices(List<Service> services, Service service) {
        ...
        public static bool same(Service service1, Service service2) {
        ...
        public static bool sameName(Service service1, Service service2) {
            return service1.name == service2.name;
        }

        public static Service toService(Api api) {
        ...
        public static Route toRoute(Api api, string serviceName) {
            var route = new Route();
            route.name = api.name;
        ...
        public static List<Plugin> toPlugins(Api api) {
            var plugins = new List<Plugin>();
            if(api.plugins != null) {
        ...
        public static Plugin toPlugin(OldPlugin oldPlugin, string routeName) {
